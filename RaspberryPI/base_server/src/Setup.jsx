...
// The RaspberryPI IP because Express.js does not like hostnames
...
class Setup extends Component {
  ...
  async playSongFromResult() {
    axios.post(`http://${raspberrypiIP}:5000/play?id=${this.state.track.id}`).catch((error) => console.log(error));
  }
  
  async prevSong() {
    axios.post(`http://${raspberrypiIP}:5000/prev`).catch((error) => console.log(error));
  }  
  
  async skipSong() {
    axios.post(`http://${raspberrypiIP}:5000/skip`).catch((error) => console.log(error));
  }
  ...
  render() {
    ...
    if (this.state.track !== null) {
      card = (
          ...
          <img src={this.state.track.album.images[0].url} alt="music cover" className="corner-round-1 img-cover" />
          </div>
      );
    } else {
      card = <p hidden={true} />;
      ...
      <div className="grid">
        ...
        <div className="search">
            <form>
                ...
                <input className="corner-round-1" type="text" name="search" id="searchbar" 
          ...
          <footer>
            <button className="button-style button-control" type="button"
            onClick={() => this.prevSong()}>
              <svg role="img" height="16" width="16" viewBox="0 0 16 16"><path d="M3.3 1a.7.7 0 01.7.7v5.15l9.95-5.744a.7.7 0 011.05.606v12.575a.7.7 0 01-1.05.607L4 9.149V14.3a.7.7 0 01-.7.7H1.7a.7.7 0 01-.7-.7V1.7a.7.7 0 01.7-.7h1.6z"></path></svg>
            </button>
            <button className="button-style button-control" type="button"
            ...
            onClick={() => this.skipSong()}>
              <svg role="img" height="16" width="16" viewBox="0 0 16 16" ><path d="M12.7 1a.7.7 0 00-.7.7v5.15L2.05 1.107A.7.7 0 001 1.712v12.575a.7.7 0 001.05.607L12 9.149V14.3a.7.7 0 00.7.7h1.6a.7.7 0 00.7-.7V1.7a.7.7 0 00-.7-.7h-1.6z"></path></svg>
            </button>
          </footer>
          <div className="text-to-upper grid">
            <h1>NFC</h1>
            <div id="status" className={this.state.nfc_ready === "true"? "nfc_available corner-round-1" : "nfc_unavailable corner-round-1"}></div>
            <div className="editing">
                <button className="button-style" type="button">Clear</button>
                <button className="button-style" type="button">Write</button>
            </div>
          </div>
      </div>
    );
