...
class Setup extends Component {
  ...
  async playSongFromResult() {
    axios.post(`http://localhost:5000/play?id=${this.state.track.id}`).catch((error) => console.log(error));
  }
  
  // play previous Song
  async prevSong() {
    axios.post(`http://localhost:5000/prev`).catch((error) => console.log(error));
  }  
  
  // skip to the next Song
  async skipSong() {
    axios.post(`http://localhost:5000/skip`).catch((error) => console.log(error));
  }
  ...
  async writeToBroker() {
    ...
    if(this.state.track !== null){
      const payloads = [
        { topic: 'ID', messages: this.state.track.id, partition: 0 },
      ];
      ...
      <div className="grid">
        ...
        <div className="text-to-upper grid">
            ...
            <div className="editing">
              ...
              <button className="button-style" type="button" onClick={() => this.writeToBroker()}>Write</button>
            </div>
          </div>
      </div>
    );
